...
export class TodoistTest {
  ...
  public async testAll(verbose: boolean): Promise<boolean> {
    ...
    if (verbose) {
      if (!testResults[0]) {
        console.log(`[👎] Something went wrong with the Task objects`);
      }
      if (!testResults[1]) {
        console.log(`[👎] Something went wrong with the Section objects`);
      }
      if (!testResults[2]) {
        console.log(`[👎] Something went wrong with the Project objects`);
      }
      if (summedResults) {
        console.log(
          `[👏] The global Todoist API test has completed succesfully`,
        );
      }
    }
  ...
  private async testSection(verbose: boolean) {
    if (verbose) {
      console.log(`[⏰] Creating temporary Project for the Section test`);
    }
    const newProject = await this.todoistAPI.addProject({ name: `Test` });

    if (verbose) console.log(`[🧐] Inserting Section object`);
    const newSection = await this.todoistAPI.addSection(
    ...
    if (verbose) {
      console.log(`[⏰] Cleaning-up temporary Project for the Section test`);
    }
