  ...
  public async executeProtocol() {
    ...
    for (let i = 0; i < this.directoryPaths.length; i++) {
      ...
      for (const entry of children) {
        ...
        if (stat.isDirectory && isGitSync(entry.path)) repositories.push(entry.path);
      }
    }
    ...
    for (let i = 0; i < repositories.length; i++) {
      ...
      else await this.repositoryDatabase.insertOne({ repository, files, creation });
    }
    ...
    uniques.forEach((unique) => {
      const datapoints = results.filter((result) => result.repository === unique);
      ...
      for (let i = 0; i < datapoints.length - 1; i++) {
        ...
        if (JSON.stringify(upcoming.files) !== JSON.stringify(current.files)) return;
      }
    ...
    if (unchanged.length > 0) {
      ...
      const message = unchanged.length > 1 ? `${part} have contained uncommited changes for more than a week!` : `${part} has contained uncommited changes for more than a week!`;

      sendDirectMessage(this.systemSettings.discordId!, message);
      sendDirectMessage(this.systemSettings.discordId!, `You might want to discard or commit these changes`);
